 ...
 * 
 * 用户下载依赖包到本地后，需要确定在下载过程中没有出现错误，所以在下载完成之后需要在本地在计算一次文件的 hash 值，
 * 如果两个 hash 值是相同的，则确保下载的依赖是完整的，如果不同，则进行重新下载。
 */
 ...
 *            校验包的完整性
 *            校验不通过：重新下载    
 *            校验通过：将下载的包复制到 npm 缓存目录
 *                     将下载的包按照依赖结构解压到 node_modules
 * 
 *        (1-) 存在缓存：将缓存按照依赖结构解压到 node_modules    
 * 
 *    - 将包解压到 node_modules
 *    - 将包解压到 node_modules
 * 
...
/**
 * yarn.lock 和 package-lock.json 一些区别
 * 
 * (1) package-lock.json 使用的是 json 格式，yarn.lock 使用的是一种自定义格式
 * (2) yarn.lock 中子依赖的版本号不是固定的，意味着单独又一个 yarn.lock 确定不了 node_modules 目录结构，
 *    还需要和 package.json 文件进行配合。而 package-lock.json 只需要一个文件即可确定。
 */
