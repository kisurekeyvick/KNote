/**
 * https://github.com/Cosen95/blog/issues/8
 * 
 * 你不知道的npm
 */
...
/**
 * 前言：
 * 大多数同学对于 npm 的使用却只停留在了npm install这里。（相信删除 node_modules 文件夹，重新执行 npm install）
 */
 ...
 * 
 * - dependencies
 * 应用依赖，或者叫做业务依赖，是我们最常用的一种。这种依赖是应用发布后上线所需要的，
 * 也就是说其中的依赖项属于线上代码的一部分。比如框架react，第三方的组件库ant-design等
 * 
 * 
 * - devDependencies
 * 开发环境依赖。这种依赖只在项目开发时所需要，比如构建工具webpack、gulp，单元测试工具jest、mocha等。
 * 
 * 
 * - peerDependencies
 * 听起来可能没有那么好理解，举个例子来说明下。antd@3.19.5只是提供了一套基于react的ui组件库，
 * 但它要求宿主环境需要安装指定的react版本，所以你可以看到 node_modules 中 antd 的package.json中有这么一项配置:
    "peerDependencies": {
      "react": ">=16.0.0",
      "react-dom": ">=16.0.0"
    },
 ...
 * 在安装插件的时候，peerDependencies 在npm 2.x和npm 3.x中表现不一样。
 * npm2.x 会自动安装同等依赖，npm3.x 不再自动安装，会产生警告！手动在package.json文件中添加依赖项可以解决。
 * 
 * 
 * - optionalDependencies
 * 可选依赖。这种依赖中的依赖包即使安装失败了，也不影响整个安装的过程。
 * 需要注意的是，optionalDependencies会覆盖dependencies中的同名依赖包，所以不要在两个地方都写。
 * 
 * 在实际项目中，如果某个包已经失效，我们通常会寻找它的替代方案。不确定的依赖会增加代码判断和测试难度，所以这个依赖项还是尽量不要使用。
 * 
 * 
 * - bundledDependencies / bundleDependencies
 * 打包依赖。如果在打包发布时希望一些依赖包也出现在最终的包里，那么可以将包的名字放在bundledDependencies中，
 * bundledDependencies 的值是一个字符串数组
    {
      "name": "sequelize-test",
      "version": "1.0.0",
      "description": "",
 ...
 * npm 包使用语义化版控制，我们可安装一定版本范围的 npm 包，npm 会选择和你指定的版本相匹配 的 (latest)最新版本安装。
 * 
 * 
 * npm 采用了semver规范作为依赖版本管理方案。
 * 版本号由三部分组成:主版本号、次版本号、补丁版本号。变更不同的版本号，代表不同的意义:
 * 
 ...
 * 
 * 
 * "typescript": "^3.4.3" - 【表示兼容补丁和小版本更新的版本号】
 * 
 *        举例子："^3.4.3" 等价于 `">= 3.4.3 < 4.0.0"。即只要最左侧的 "3" 不变，其他都可以改变。所以 "3.4.5", "3.6.2" 都可以兼容。
 ...
 * 
 * 
 * "mime-types": "~2.1.24" - 【表示只兼容补丁更新的版本号】
 * 关于 ~ 的定义分为两部分：如果列出了小版本号（第二位），则只兼容补丁（第三位）的修改；如果没有列出小版本号，则兼容第二和第三位的修改
 * 
 *        举例子："~2.1.24" 列出了小版本号 "1"，因此只兼容第三位的修改，等价于 ">= 2.1.24 < 2.2.0"
 ...
 * 【除了上面的x、X还有*和(空)，这些都表示使用通配符的版本号，可以匹配任何内容】
 * 
 *       举例子："*" 、"x" 或者 （空） 表示可以匹配任何版本
 ...
 * (3) npm install 原理分析
 * 
 * 执行npm install后，依赖包被安装到了node_modules中。
 * 
 * - 嵌套结构
 * 在 npm 的早期版本中，npm 处理依赖的方式简单粗暴，以递归的方式，
 ...
 * 举个例子，我们的项目ts-axios现在依赖了两个模块: axios、body-parser
    ...
    但是如果你依赖的模块非常之多，你的 node_modules 将非常庞大，嵌套层级非常之深，会造成如下后果：
 ...
 * 在执行 npm install 后将得到下面的目录结构：【3.X以后的npm加载模块后的结构.png】
 * 
 * 对应的，如果我们在项目代码中引用了一个模块，模块查找流程如下：
 *      (1) 在当前模块路径下搜索
 *      (2) 在当前模块 node_modules 路径下搜索
 *      (3) 在上级模块的 node_modules 路径下搜索
 *      ...
 *      (final) 直到搜索到全局路径中的 node_modules
 * 
 * 假设我们又依赖了一个包 axios2@^0.19.0，而它依赖了包 is-buffer@^2.0.3，则此时的安装结构是下面这样的：
 * 【3.Xnpm结构引发的问题.png】
 * 所以我们在 package.json 通常只会锁定大版本，这意味着在某些依赖包小版本更新后，同样可能造成依赖结构的改动，依赖结构的不确定性可能会给程序带来不可预知的问题。
 * 
 * 
 * 
 * - package-lock.json
 * npm install 的不确定性问题，在 npm 5.x 版本新增了 package-lock.json 文件，而安装方式还沿用了 npm 3.x 的扁平化的方式。
 * 
 ...
 * (1) version: 包唯一的版本号
 * (2) resolved: 安装来源
 * (3) integrity: 表明包完整性的 hash 值（验证包是否已失效）
 * (4) requires: 依赖包所需要的所有依赖项，与子依赖的 package.json 中 dependencies的依赖项相同。
 * (5) dependencies: 依赖包node_modules中依赖的包，与顶层的dependencies一样的结构
 * 
 * 
 ...
 * 
 * (2) 传入参数
 * 在原有脚本后面加上 -- 分隔符, 后面再加上参数，就可以将参数传递给 script 命令了，
 * 比如 eslint 内置了代码风格自动修复模式，只需给它传入 -–fix 参数即可，我们可以这样写:
    "scripts": {
 ...
 * 第一个元素是 process.execPath，表示启动 node 进程的可执行文件的绝对路径名。
 * 第二个元素为当前执行的 JavaScript 文件路径。剩余的元素为其他命令行参数。
 * 
 * 
 * (3) 执行顺序
 * 如果 npm 脚本里面需要执行多个任务，那么需要明确它们的执行顺序。
 * 如果是串行执行，即要求前一个任务执行成功之后才能执行下一个任务。使用&&符号连接。：npm run script1 && npm run script2
 * 
 * 串行命令执行过程中，只要一个命令执行失败，则整个脚本将立刻终止
 * 
 * 如果是并行执行，即多个任务可以同时执行。使用&符号来连接：npm run script1 & npm run script2
 * 
 * 
 * (4) 钩子
 ...
 * 
 * 比如，在用户执行npm run build的时候，会自动按照下面的顺序执行：npm run prebuild && npm run build && npm run postbuild
 * 
 ...
 * 在执行npm run脚本时，npm 会设置一些特殊的env环境变量。其中 package.json 中的所有字段，都会被设置为以npm_package_ 开头的环境变量。
 * 比如 package.json 中有如下字段内容：
    {
      "name": "sequelize-test",
      "version": "1.0.0",
 ...
 * 那么，变量npm_package_name返回sequelize-test，变量npm_package_description返回sequelize测试。
 * 
 ...
 * npm 配置 - 优先级
 * 
 * npm 从以下来源获取配置信息（优先级由高到低）：命令行 > 环境变量 > npmrc文件 > 默认配置
 * 
 * (1) 命令行
 * npm run dev --foo=bar
 ...
 * 比如，环境变量中的npm_config_foo=bar 将会设置配置参数 foo 的值为 "bar"。
